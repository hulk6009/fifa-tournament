...
const renderFixtureTable = (groupKey, fixtures) => {
  const tbody = document.querySelector(`#fixtureTable-${groupKey} tbody`);
  tbody.innerHTML = '';
  fixtures.forEach(f => {
    const tr = document.createElement('tr');
    const played = hasScore(f);
    const score = played ? `${f['Home Score']} - ${f['Away Score']}` : 'vs';

    [f['Matchday'], f['Home Player'], score, f['Away Player']].forEach(val => {
      const td = document.createElement('td');
      td.textContent = val;
      tr.appendChild(td);
    });
    ...
    badge.className = `status-badge ${played ? 'status-completed' : 'status-scheduled'}`;
    badge.textContent = played ? 'Played' : 'Upcoming';
